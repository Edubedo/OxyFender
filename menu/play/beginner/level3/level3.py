...
class Level3Beginner:
    def __init__(self, name, dificultadNivel, id, configLanguage, datosLanguage, volumen):
        ...
        self.imagen_fondo = pygame.image.load(join("assets", "img", "Background", "menu", "fondo_city.png")).convert()
        ...
        self.tmx_mapa_2 = load_pygame(join("assets", "maps", "beginner", "level3", "ciudad.tmx"))
    ...
    def run(self):
        ...
        pygame.mixer.music.load(join("assets", "audio", "niveles", "level3.mp3"))
    ...
    def pantallaBasuras(self):
        ...
        while banderaEjecutandoNivel2:
            ...
            for layer in self.tmx_filtroUnoNivel2.visible_layers:
                if isinstance(layer, pytmx.TiledTileLayer):
                    for x, y, gid in layer:
                        ...
                        if tile:
                            tile_rect = pygame.Rect(x * self.tmx_filtroUnoNivel2.tilewidth + offset_x, y * self.tmx_filtroUnoNivel2.tileheight + offset_y, self.tmx_filtroUnoNivel2.tilewidth, self.tmx_filtroUnoNivel2.tileheight)
                            if tile_rect.collidepoint(mouse_pos) and layer.name in capas_basura:
                                hand_cursor = True
            ...
            # Check if all trash layers are removed
            if not capas_basura:
                self.arreglo = True
